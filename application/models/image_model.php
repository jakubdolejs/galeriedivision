...
class Image_model extends GD_Model {
    ...
    public function get_list() {
        $this->db->select("image.id, artist_id, concat(artist.name,' ',artist.surname), image.version",false)
            ->from("image")
    ...
    public function get($image_id) {
        $this->db->select("image.id, work_width, work_height, work_depth, work_creation_year, artist_id, concat(artist.name,' ',artist.surname) as 'artist_name', exhibition_id, image_translation.title, image_translation.description, image_translation.lang, gallery_id, version",false)
            ->from("image")
            ->join("image_artist JOIN artist ON image_artist.artist_id = artist.id","image.id = image_artist.image_id","left")
    ...
    public function get_artist_images($artist_id,$gallery_id=null) {
        $this->db->select("image.id, artist_id, concat(artist.name,' ',artist.surname) as 'name', version",false)
            ->from("image")
            ->join("image_artist JOIN artist ON image_artist.artist_id = artist.id","image.id = image_artist.image_id","left");
        if ($gallery_id) {
